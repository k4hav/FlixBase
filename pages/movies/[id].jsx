      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 pt-20 pb-20">
        ...
        <div className="flex flex-col md:flex-row gap-8 lg:gap-12">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* ── AVAILABLE ON ── */}
            {movie.platforms && Array.isArray(movie.platforms) && movie.platforms.length > 0 && (
              <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.25 }}
                ...
                <div className="flex flex-wrap gap-2">
                  {movie.platforms.map((p, i) => (
                    <motion.a key={i}
                      href={p.url ? p.url + encodeURIComponent(movie.title) : '#'}
                      target={p.url ? '_blank' : '_self'}
                      rel="noopener noreferrer"
                      whileHover={{ scale:1.05, y:-2 }} whileTap={{ scale:0.96 }}
                      className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold transition-all"
                      style={{
                        background: (p.color||'#c9a84c')+'20',
                        border: `1px solid ${p.color||'#c9a84c'}50`,
                        color: p.color||'#c9a84c',
                        textDecoration: 'none',
                        boxShadow: `0 4px 20px ${p.color||'#c9a84c'}15`,
                      }}>
                      Watch on {p.name}
                      {p.url && <ExternalLink size={10} style={{ opacity:0.6 }} />}
                    </motion.a>
                  ))}
                </div>
              </motion.div>
